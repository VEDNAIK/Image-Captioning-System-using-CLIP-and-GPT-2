...
class CaptioningModel(nn.Module):
    ...
    def forward(self, img_embedded, temperature=1.0):
        ...
        with torch.no_grad():
            ...
            img_mapped = self.mapping(img_embedded) # mapping
            ...
            start_emb = torch.cat([img_mapped, bos_emb], dim=0) # obtain start embedding
            ...
            for _ in range(self.max_len):
                ...
                # See https://huggingface.co/docs/transformers/model_doc/gpt2.
                pos_emb = self.text_decoder.model.transformer.wpe(torch.arange(emb.size(0)).to(self.device)) # obtain position embedding
                emb = emb + pos_emb# add position embedding to token embedding

                # TODO: Generate the next token. See https://huggingface.co/docs/transformers/model_doc/gpt2.
                pred = self.text_decoder(emb.unsqueeze(0)).squeeze(0) # generate the next token
    ...
    def train_forward(self, img_emb, trg_cap, att_mask):
        ...
        # TODO: Map image to text space. See class Mapping in this file.
        img_mapped = self.mapping(img_emb, train_mode=True) # mapping. Now we are training the model, so the train_model parameter should be set to True
        ...
        # TODO: Obtain position embedding and add it into token embedding. The position embedding is stored as an Embedding layer named "wpe" in GPT-2.
        # See https://huggingface.co/docs/transformers/model_doc/gpt2.
        pos_emb = self.text_decoder.model.transformer.wpe(torch.arange(x.size(1)).to(self.device)) # obtain position embedding
        pos_emb = pos_emb.expand_as(x)
        x = x + pos_emb # add position embedding to token embedding
        ...
        loss = self.criterion(res.transpose(1, 2), y) # cross-entropy loss
